...
const getAll = catchError(async(req, res) => { // Controlador para leer todos los registros
    const results = await movieModel.findAll({include: [Genre]}); // el metodo .finAll() para retornar toda la informacion
...
const create = catchError(async(req, res) => { // Controlador para crear registros
    const result = await movieModel.create(req.body); // el metodo .create(<registro a crear>) para crear
...
const getOne = catchError(async(req, res) => { // Controlador para leer un registro por id
    ...
    const result = await movieModel.findByPk(id, {include: [Genre]}); // El metodo .findByPk(<id del registro a actualizar>) para actualizar por id
...
const remove = catchError(async(req, res) => { // Controlador para eliminar registros por id
    const { id } = req.params; // Accedo al id por parametros
    const remove = await movieModel.destroy({ where: {id} }); // El metodo .destroy({ where: {<id del registro a eliminar>} }) para eliminar por id
